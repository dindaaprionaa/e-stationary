    ...
    public function checkout($action = '')
{
    ...
    switch ($action) {
        default:
            $coupon = $this->input->post('coupon_code') ?? $this->session->userdata('_temp_coupon');
            $quantity = $this->input->post('quantity') ?? $this->session->userdata('_temp_quantity');
